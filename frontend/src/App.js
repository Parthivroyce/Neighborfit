...
function App() {
  ...
  const exportPDF = () => {
    ...
    autoTable(doc, {
      head: [["Neighborhood", "Budget", "Commute", "Noise", "Pets", "Green"]],
      body: result.map(n => [
        n.name,
        n.budget,
        n.commute_time,
  ...
  return (
    <div className={`${darkMode ? 'bg-gray-900 text-white' : 'bg-blue-50 text-black'} min-h-screen flex flex-col items-center justify-center p-6`}>
      ...
      {!token ? (
        <form onSubmit={handleLogin} className="bg-white shadow-lg rounded-xl px-8 pt-6 pb-8 mb-6 w-full max-w-md space-y-4 border border-blue-100">
          <h2 className="text-xl font-semibold text-blue-600">🔐 Login</h2>
          <input type="text" name="username" placeholder="Username" value={loginData.username} onChange={handleLoginChange} className="w-full border rounded p-2" />
          <input type="password" name="password" placeholder="Password" value={loginData.password} onChange={handleLoginChange} className="w-full border rounded p-2" />
        ...
        <>
          ...
          <form onSubmit={handleSubmit} className="bg-white shadow-lg rounded-2xl px-8 pt-6 pb-8 mb-6 w-full max-w-xl space-y-6 border border-blue-100">
            <h2 className="text-2xl font-semibold text-blue-600 flex items-center gap-2">🎯 Match Your Preferences</h2>
